...
ts, x, data = EMD.sawtooth()
x = imfs[2]
outfolder = "/home/crem_fe/Documents/presentations/MPI_20201125/"

function animate_sifting(ts, x, outfolder)
    maxcol, mincol, meancol = (:green, :orange, :blue)
    mkpath(outfolder)
    maxes = Int[]
    mins = Int[]
    yvec = Node(x)
    ...
    st = Stepper(scene, joinpath(outfolder, "sift_animation/"))
    step!(st)
    ...
    display(scene)
    framerate = 5
    record(scene, joinpath(outfolder, "emd_sifting_$(framerate).mp4"), framerate=framerate) do io
...
imfs = EMD.emd(x,ts)
sc, layout = layoutscene(30, resolution=(1200,700), backgroundcolor= RGBf0(0.98,0.98,0.98))
display(sc)
